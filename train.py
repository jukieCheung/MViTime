from mvitime import mobilevit_xxs
import torch
...
data = np.load('/root/mvitime/data/edf78.npz')
orig_x = data['x']
...
net = mobilevit_xxs(classification=False).to(device)
...
for epoch in range(epochs):
    ...
    with tqdm.tqdm(total=iter_per_epoch) as pbar:
        ...
        for X, y in train_iter:
            ...
            trans = torch.tensor(trans, dtype=torch.float, device="cuda:0")
...
net = mobilevit_xxs(classification=True).to('cuda')
net = nn.DataParallel(net)
...
for epoch in range(epochs_t):
    ...
    with tqdm.tqdm(total=iter_per_epoch) as pbar:
        for X, y in train_iter:
            ...
            evaluation.append((predicted == torch.argmax(y,dim=1)).tolist())
            optimizer.zero_grad()
            l = criterion(output,torch.tensor(y,dtype=torch.float32).cuda())
...
# =========================test
model = mobilevit_xxs(classification=True).to('cuda')
